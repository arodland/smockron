...
Socket.prototype.dispatchMessage = function(msg) {
	if (msg.type == 0x0001) {
		this.emit('hello', message.Hello.parse(msg));
	} else if (msg.type == 0x0101) {
		this.emit('accounting', message.Accounting.parse(msg));
	} else if (msg.type == 0x8001) {
		this.emit('delayUntil', message.DelayUntil.parse(msg));
	} else if (msg.type == 0x8101) {
		this.emit('sync', message.Sync.parse(msg));
	} else {
