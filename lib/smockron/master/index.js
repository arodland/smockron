...
function Master(config) {
  ...
  this.server.on('resync', this._onResync.bind(this));
};
...
Master.prototype._onResync = function(msg) {
  var domain = this.domains[msg.domain];
  var self = this;
  if (!domain) {
    console.warn("Received resync for unknown domain", msg.domain);
    return;
  }

  self.dataStore.getAllForDomain(msg.domain).then(function (entries) {
    self.server.sendControl({
      domain: msg.domain,
      command: 'RESYNCSTART'
    });
